...
{  
   ...
   //USART2_TX   PA2
	...
	USART2_InitStructure.USART_BaudRate = bound;                     //波特率一般设置为9600;
	...
	USART_Init(USART2, &USART2_InitStructure); //初始化串口2
...
{  
	...
	va_list ap; 
	va_start(ap,fmt);
	vsprintf((char*)USART2_TX_BUF,fmt,ap);  //使用参数列表发送格式化输出到字符串
	va_end(ap);
    //printf("ok\r\n");
...
void usart2_rx(u8 enable)
{
	 ...
	 {
		 USART2_InitStructure.USART_Mode = USART_Mode_Tx;//只发送 
	 }
